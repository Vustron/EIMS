...
Class Master extends DBConnection {
	...
	function fetchInventory() {
        $query = $this->conn->query("SELECT i.*,p.title as product,p.author from `inventory` i inner join `products` p on p.id = i.product_id order by unix_timestamp(i.date_created) desc ");
        while($row = $query->fetch_assoc()) {
            $sold = $this->conn->query("SELECT SUM(ol.quantity) as sold FROM order_list ol inner join orders o on o.id = ol.order_id where ol.product_id='{$row['id']}' and o.`status` != 4 ");
	...
	function delete_user(){
	    ...
	    if(isset($_POST['id'])){
	        ...
	        $delete = $this->conn->query("DELETE FROM `clients` WHERE `id` = '{$id}' ");
