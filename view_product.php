...
<?php 
 ...
 if($products->num_rows > 0){
     ...
     while($ir = $inventory->fetch_assoc()){
        $sold = $conn->query("SELECT SUM(ol.quantity) as sold FROM order_list ol inner join orders o on o.id = ol.order_id where ol.product_id='{$ir['id']}' and o.`status` != 4 ");
        $sold = $sold->num_rows > 0 ? $sold->fetch_assoc()['sold'] : 0;
        $avail = $ir['quantity'] - $sold;
        $ir['avail'] = $avail;
        $inv[] = $ir;
     }             
 }
        ...
        <div class="row gx-4 gx-lg-5 align-items-center">
           ...
           <?php if ($avail == 0){
            echo "<div class='col-md-6'>
                    <h1 class='display-5 fw-bolder border-bottom border-top border-primary pb-1'>OUT OF STOCK</h1>
                    <br>
                  </div>";                 
            } else {
            ?> 
            ...
            <div class="d-flex">
                ...
                <input class="form-control text-center me-3" id="inputQuantity" type="num" value="1" style="max-width: 3rem" name="quantity" min="1" max=<?php echo $inv[0]['quantity'] ?>/>
